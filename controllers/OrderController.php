    ...
    switch ($handler) {
        ...
        */
        case "deleteOrder":
            ...
            if(!isOrderedByMe($userIdx, $orderIdx)){
                $res->isSuccess = FALSE;
                $res->code = 2005;
                $res->message = "구매 취소 권한이 없습니다";
                echo json_encode($res, JSON_NUMERIC_CHECK);
                break;
            }

            if(isDelivered($userIdx, $orderIdx)){
                $res->isSuccess = FALSE;
                $res->code = 2006;
                $res->message = "이미 배송된 작품입니다";
                echo json_encode($res, JSON_NUMERIC_CHECK);
                break;
            }
        ...
        */
        case "changeOrder":
            ...
            if(!isDelivered($userIdx, $orderIdx)){
                $res->isSuccess = FALSE;
                $res->code = 2006;
                $res->message = "배송되지 않은 상품입니다";
                echo json_encode($res, JSON_NUMERIC_CHECK);
                break;
            }

            changeOrder($orderIdx);
            $res->isSuccess = TRUE;
            $res->code = 1000;
            $res->message = "교환 신청 성공";
            echo json_encode($res, JSON_NUMERIC_CHECK);
            break;
        ...
        */
        case "refundOrder":
            ...
            refundOrder($orderIdx);
            $res->isSuccess = TRUE;
            $res->code = 1000;
            $res->message = "환불 신청 성공";
            echo json_encode($res, JSON_NUMERIC_CHECK);
            break;
